...
export async function getNews() {
  try {
    ...
    // Scrape images for first 2 items only (to avoid Vercel 10s timeout)
    const scrapePromises = itemsToProcess.slice(0, 2).map(async (item) => {
    ...
    const remainingItems = itemsToProcess.slice(2).map((item) => ({
      id: item.url,
      title: item.title,
      url: item.url,
